...
namespace Kits.ClientKit.Collections
{
    ...
    public static class Perlin
    {
        ...
        public const float FbmNorm = 1 / 0.75f;

        public static float Noise(float x)
        {
            int xi = Mathf.FloorToInt(x) & 0xff;
            x -= Mathf.Floor(x);
            float u = Fade(x);
            return Lerp(u, Grad(s_Perm[xi], x), Grad(s_Perm[xi + 1], x - 1)) * 2;
        }
        ...
        {
            int xi = Mathf.FloorToInt(x) & 0xff;
            int yi = Mathf.FloorToInt(y) & 0xff;
            x -= Mathf.Floor(x);
            y -= Mathf.Floor(y);
            float u = Fade(x);
            float v = Fade(y);
            int a = (s_Perm[xi] + yi) & 0xff;
            int b = (s_Perm[xi + 1] + yi) & 0xff;
        ...
        public static float Noise(float x, float y, float z)
        {
            int xi = Mathf.FloorToInt(x) & 0xff;
            int yi = Mathf.FloorToInt(y) & 0xff;
            int zi = Mathf.FloorToInt(z) & 0xff;
            x -= Mathf.Floor(x);
            y -= Mathf.Floor(y);
            z -= Mathf.Floor(z);
            float u = Fade(x);
            ...
            int a = (s_Perm[xi] + yi) & 0xff;
            int b = (s_Perm[xi + 1] + yi) & 0xff;
            int aa = (s_Perm[a] + zi) & 0xff;
            int ba = (s_Perm[b] + zi) & 0xff;
            int ab = (s_Perm[a + 1] + zi) & 0xff;
            int bb = (s_Perm[b + 1] + zi) & 0xff;
