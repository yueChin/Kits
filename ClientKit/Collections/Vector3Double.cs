...
namespace Kits.ClientKit.Collections
{
    ...
    public struct Vector3Double
    {
        ...
        public static Vector3Double v3Zero { get; set; }
        ...
        {
            return (double)Vector3Double.GetSqrMagnitude(lhs - rhs) < 0.0 / 1.0;
        }
        ...
        public static Vector3Double SmoothDamp(Vector3Double current, Vector3Double target, ref Vector3Double currentVelocity, double smoothTime, double maxSpeed, double deltaTime)
        {
            ...
            Vector3Double vector31 = target;
            double maxLength = maxSpeed * smoothTime;
            Vector3Double vector32 = Vector3Double.ClampMagnitude(vector, maxLength);
            target = current - vector32;
            Vector3Double vector33 = (currentVelocity + num1 * vector32) * deltaTime;
            currentVelocity = (currentVelocity - num1 * vector33) * num3;
            Vector3Double vector34 = target + (vector32 + vector33) * num3;
            if (Vector3Double.Dot(vector31 - current, vector34 - vector31) > 0.0)
            {
                vector34 = vector31;
                currentVelocity = (vector34 - vector31) / deltaTime;
            }
            return vector34;
        }
        ...
        {
            double num = Vector3Double.GetMagnitude(value);
        ...
        {
            return MathHelper.Acos(MathHelper.Clamp(Vector3Double.Dot(from.normalized, to.normalized), -1d, 1d)) * 57.29578d;
        }
        ...
        public static double GetMagnitude(Vector3Double a)
        {
            return System.Math.Sqrt(a.x * a.x + a.y * a.y + a.z * a.z);
        }

        public static double GetSqrMagnitude(Vector3Double a)
        {
        ...
        {
            return new Vector3Double(MathHelper.Min(lhs.x, rhs.x), MathHelper.Min(lhs.y, rhs.y), MathHelper.Min(lhs.z, rhs.z));
        }

        public static Vector3Double Max(Vector3Double lhs, Vector3Double rhs)
        {
            return new Vector3Double(MathHelper.Max(lhs.x, rhs.x), MathHelper.Max(lhs.y, rhs.y), MathHelper.Max(lhs.z, rhs.z));
        }
        ...
        {
            return MathHelper.Acos(MathHelper.Clamp(Vector3Double.Dot(from.normalized, to.normalized), -1d, 1d));
        }
    }
}