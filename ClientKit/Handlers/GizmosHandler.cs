...
{
    public class GizmosHandler
    {
        public static void DrawFrustum(FrustumPlanes frustum)
        {
            if (frustum.zFar <= 0)
                return;
            Vector3 p0 = new Vector3(frustum.left, frustum.top, frustum.zFar);
            Vector3 p1 = new Vector3(frustum.right, frustum.top, frustum.zFar);
            Vector3 p2 = new Vector3(frustum.right, frustum.bottom, frustum.zFar);
            Vector3 p3 = new Vector3(frustum.left, frustum.bottom, frustum.zFar);
            Gizmos.DrawLine(p0, p1);
            Gizmos.DrawLine(p1, p2);
            Gizmos.DrawLine(p2, p3);
            Gizmos.DrawLine(p3, p0);
            float f = frustum.zNear > 0 && frustum.zNear < frustum.zFar ? frustum.zNear / frustum.zFar : 0;
            f = 1 - f;
        ...
        public static float LodArc(Vector3 pos)
        {
            ...
            if(UnityEditor.SceneView.currentDrawingSceneView != null)
            {
                ...
                float dis = Vector3.SqrMagnitude(cam.transform.position - pos);
        ...
        public static void MarkTransform(UnityEngine.Transform trans, float size)
        {
        ...
        //屏幕像素大小与本地单位大小比值
        public static float FactorToPixel(UnityEngine.Camera camera, UnityEngine.Transform trans, Vector3 localPos = new Vector3())
        {
        ...
        //本地单位大小与屏幕像素大小比值
        public static float FactorFromPixel(UnityEngine.Camera camera, UnityEngine.Transform trans, Vector3 localPos = new Vector3())
        {
            Vector3 lp1 = trans ? trans.localToWorldMatrix.MultiplyPoint(localPos) : localPos;
        ...
        public static float FactorToSceneViewPixel(UnityEngine.Transform trans, Vector3 localPos = new Vector3())
        {
#if UNITY_EDITOR
            UnityEditor.SceneView scene = UnityEditor.SceneView.currentDrawingSceneView;
            if (scene)
        ...
        public static UnityEngine.Camera GetSceneViewCamera()
        {
#if UNITY_EDITOR
            UnityEditor.SceneView scene = UnityEditor.SceneView.currentDrawingSceneView;
            if (scene)
        ...
        public static float FactorFromSceneViewPixel(UnityEngine.Transform trans, Vector3 localPos = new Vector3())
        {
#if UNITY_EDITOR
            UnityEditor.SceneView scene = UnityEditor.SceneView.currentDrawingSceneView;
            if (scene)
