...
namespace Kits.DevlpKit.Helpers.SecurityHelpers.CRC32
{
    ...
    {
        private const int c_CachedBytesLength = 0x1000;
        private static readonly byte[] s_CachedBytes = new byte[c_CachedBytesLength];
        ...
        public static int GetCrc32(Stream stream)
        {
            ...
            while (true)
            {
                int bytesRead = stream.Read(s_CachedBytes, 0, c_CachedBytesLength);
                if (bytesRead > 0)
                {
            ...
            int result = (int) s_Algorithm.HashFinal();
            s_Algorithm.Initialize();
            Array.Clear(s_CachedBytes, 0, c_CachedBytesLength);
            return result;
        }
