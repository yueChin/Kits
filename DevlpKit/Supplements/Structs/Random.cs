...
namespace Kits.DevlpKit.Supplements.Structs
{
    ...
    public struct Random
    {
        ...
        private static Random s_Static = Create((uint)(DateTime.Now.Ticks & 0x_FFFF_FFFF));
        ...
        {
            s_Static.Next();
            return new Random { seed = ~s_Static.seed };
        }
        ...
        public float Normal(float averageValue, float standardDeviation)
        {
            ...
            return averageValue + standardDeviation * (float)
                (
                    Math.Sqrt(-2 * Math.Log(1 - Next01())) * Math.Sin(MathHelper.TwoPi * Next01())
                );
        }
